...
from slab import SlabFile, get_next_filename, AttrDict
...
def save_2D_data_h5(data_path, filename, data, config=None):
    file_path = data_path + "\\" + get_next_filename(data_path, filename, ".h5")
...
def save_data(data_path, filename, data, config=None):
    file_path = os.path.join(data_path, get_next_filename(data_path, filename, ".h5"))
    with h5py.File(file_path, "w") as f:
        xpts = data.get("xpts", None)
        avgi = data.get("avgi", None)
        avgq = data.get("avgq", None)
