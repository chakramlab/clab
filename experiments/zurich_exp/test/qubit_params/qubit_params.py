...
def single_qubit_parameters():
    return {
        # qb drive settings
        "qb_freq": (4.0 - 4.0)
        * 1e9,  # ((4.202602999397587-4.0)*1e9,#4.212345894208691-4.0)*1e9,(4.202602999397587-4.0)*1e9,##213.3290994438175e6, #212.255227653027e6,#201.9104395175273e6,  #202.0920262028083e6,#-10.177e6, #212.2546934228453e9, #202.0920262028083e6, # 4.2124185121083 qubit 0 drive frequency in [Hz] - relative to local oscillator
        "qb_pi_len": 0.10379561412587698e-6,
        "qb_pi_amp": 0.9,
        "qb_ef_freq": (4.1042398397321005 - 4.0) * 1e9,
        "qb_ef_len": 0.037167577523869885e-6,  # 166.49042457079904e-9, #300e-9,#
        "qb_ef_amp": 1,  # 1,s
        "qb_drive_dBm_range": -30,  # 10 dBm range for the qubit drive
        "qb_resolved_pi_len": 12.438222960677162e-6,  # 3.82638253303124e-6,
        "qb_resolved_pi_amp": 0.3,  # 1,
        ...
        "qb_resolved_pulse_type": "const",  # "gaussian" or "const"
        # measurement settings
        "reset_delay": 0.3e-3,  # 300e-6,  # delay time after each measurement for qubit reset in [s]
        "cavity_reset_delay": 1.2e-3,
        "ro_len": 2e-6,  # 2us sq
        "ro_freq": (7.692399354557562 - 7.6)
        * 1e9,  # 0.09235327987160158e9, #(0.0924474192152056)*1e9, # 7.783671267760919
        "ro_amp": 0.9,  #
        "ro_delay": 0e-9,
        ...
        ###### alice
        "cav_alice_freq": (4.6487470170967375 - 4.6)
        * 1e9,  # (4.270245132001621-4.6)*1e9, #
        "cav_alice_len": 0.00002,  # 0.0001, # 20ns - 2us gaus/arb
        "cav_alice_amp": 0.1,  #
        "cav_alice_dBm_range": -30,
        ###### bob
        # "cav_alice_freq": 0.04878485781762265e9, #
        # "cav_alice_len": 32.6e-6, # 20ns - 2us gaus/arb
        # "cav_alice_amp": 0.0632455532033676, #
        # "cav_alice_dBm_range": -10,
        # "cav_bob_freq": 0.04878485781762265, #0.035954734601355e9, #
        "cav_bob_len": 20e-6,  # 20ns - 2us gaus/arb
        "cav_bob_amp": 0.9,
...
for name in sb_pulses.keys():
    for st in range(8):
        ...
        sb_pulses[name][f"f{st}g{st+1}"] = custom_gaussian_square(
            ...
            length=qubit_parameters["q0"][f"sb_f{st}g{st+1}_{name}_flat_len"]
            + qubit_parameters["q0"][f"sb_{name}_ramp_len"],
            ramp=qubit_parameters["q0"][f"sb_{name}_ramp_len"],
            amplitude=qubit_parameters["q0"][f"sb_f{st}g{st+1}_{name}_amp"],
            can_compress=True,
        )
