...
def sideband_readout_rabi(
    device_setup,
    serial_num,
    qubit_params_file_path,
    exp_id="sideband_readout_rabi",
    ...
    # we're using channel 4 but we need to call the lo sg2
    ...
    if sb_freq is None:
        sb_freq = qubit_parameters["q0"]["sb_readout_freqs"][0]
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(
            ...
            with exp.section(uid = "sb_transition_f0g1", play_after="ef_excitation" if prepare_f else None):
                exp.play(signal = "sb_drive_readout", pulse = sb_f0g1_readout, 
    ...
    sig_freq_map[serial_num][ch]["sb_drive_readout"]["frequency"] = sb_freq - lo
