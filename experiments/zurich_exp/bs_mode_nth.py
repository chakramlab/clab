...
def bs_mode_nth(
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(
            ...
            with exp.section(uid="eg", play_after="ef", on_system_grid=True):
                exp.play(
                    signal="qb_drive",  
                    pulse=ge_X180, 
                )
            
            with exp.section(uid="fe", play_after="eg", on_system_grid=True):
                exp.play(
                    signal="qb_ef_drive", 
                    pulse=ef_X180, 
                )

            with exp.section(uid="readout", play_after="fe", on_system_grid=True):
                exp.measure(
