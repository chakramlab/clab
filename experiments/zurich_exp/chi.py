...
def chi(
    ...
    swp_param=LinearSweepParameter(uid="time_swp", start=8e-9, stop=10e-6, count=10),
    length=4e-6,
    amplitude=0.3,
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(
            uid="time_sweep", parameter=swp_param, reset_oscillator_phase=True
        ):  # chunk_count= 10
            
            with exp.section(uid="cav_displacement"):
                exp.play(signal=cav_line, pulse=cav_pulse, length=length, amplitude=amplitude)

            with exp.section(uid="qubit_excitation", play_after="cav_displacement"):
                exp.play(signal="qb_drive", pulse=ge_X90)
                exp.delay(signal="qb_drive", time=swp_param)
                exp.play(signal="qb_drive", pulse=ge_X90)
