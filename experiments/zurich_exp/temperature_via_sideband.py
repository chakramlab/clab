...
def sideband_rabi(
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(
            ...
            # without f state preparation
            with exp.section(uid = "sb_transition_f0g1", play_after=None):
            ...
            with exp.section(uid = "fe_transition_0", play_after = "sb_transition_f0g1"):
                exp.play(signal = "qb_ef_drive", pulse = ef_X180)

            with exp.section(uid = "readout_0", play_after = "fe_transition_0"):
            ...
            # with f state preparation
            with exp.section(uid = "ge_excitation",  play_after='readout_0'): #alignment=SectionAlignment.RIGHT):
                exp.play(signal = "qb_drive", pulse = ge_X180)

            with exp.section(uid = "ef_excitation", play_after= "ge_excitation", on_system_grid=True):
                exp.play(signal = "qb_ef_drive", pulse = ef_X180)

            with exp.section(uid = "fe_transition_1", play_after = "sb_transition_f0g1"):
                exp.play(signal = "qb_ef_drive", pulse = ef_X180)

            with exp.section(uid = "readout_1", play_after = "fe_transition_1"):
                exp.measure(measure_signal = "measure", measure_pulse = readout_pulse,
                            acquire_signal = "acquire", integration_kernel = kernels, handle = "ac_1",
                            reset_delay = qubit_parameters["q0"]["cavity_reset_delay"], 
                            acquire_delay=qubit_parameters['q0']['acquire_delay'])
