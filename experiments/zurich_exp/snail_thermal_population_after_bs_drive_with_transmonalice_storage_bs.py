...
def snail_thermal_population_after_bs_drive_with_transmonalice_storage_bs(
    device_setup,
    serial_num,
    qubit_params_file_path,
    exp_id="snail_thermal_population_after_bs_drive_with_transmonalice_storage_bs",
    ...
    # make another sweep parameter for the ge pulse T/F
    if swp_param_ge is None:
        swp_param_ge = LinearSweepParameter(
            uid="swp_param_ge", start=0*ge_X180.length, stop=1*ge_X180.length, count=2
        )
    ge_length = swp_param_ge
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(
                uid="time_or_amp_sweep_off_resonant", parameter=swp_param_off_resonant, reset_oscillator_phase=True,
                ):  
                  
            with exp.sweep(
                uid="ge_play_swp_param", parameter=swp_param_ge, reset_oscillator_phase=True,
                ):
