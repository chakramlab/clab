...
def ramsey_ef(
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(uid="time_sweep", parameter=sweep_param):  # chunk_count= 10
            with exp.section(
            ...
            with exp.section(
                uid="ef_x90", play_after="qubit_excitation", on_system_grid=True
            ):
                exp.play(signal="qb_ef_drive", pulse=ef_X180, amplitude=0.5)
                exp.delay(signal="qb_ef_drive", time=time_swp)
                exp.play(
                    signal="qb_ef_drive", pulse=ef_X180, amplitude=0.5, phase=phase_swp
                )
            with exp.section(uid="eg_transition", play_after="ef_x90"):
                exp.play(signal="qb_drive", pulse=ge_X180)
