...
def bs_t1_test(
    ...
    swp_param=LinearSweepParameter(
        uid="time_swp_param", start=1e-9, stop=1e-3, count=50
    ),
    bs_freq=1e9,
    length=1e-6,
    bs_range=10,
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(
            ...
            with exp.section(uid="bs_swap", play_after="cav_displacement"):
                exp.play(signal="bs", pulse=bs_pulse)
                exp.delay(signal="bs", time=swp_param)
                exp.play(signal='bs', pulse=bs_pulse)

            with exp.section(uid="resolved_pi_ge", play_after="bs_swap"):
