from laboneq.simple import (
    AcquisitionType,
    Experiment,
    ExperimentSignal,
    LinearSweepParameter,
    pulse_library,
)

from .calib_settings import create_default_map_and_calibration
from .laboneq_helper import default_signal_map_and_calibration
from .load_qubit_params import load_qubit_params


def sideband_readout_spectroscopy(
    device_setup,
    serial_num,
    qubit_params_file_path,
    exp_id="sideband_readout_spectroscopy",
    freq_swp=LinearSweepParameter(
        uid="freq_swp_param", start=-700e6, stop=700e6, count=6
    ),
    acquisition_type=AcquisitionType.INTEGRATION,
    max_fock_state=1,  # only works for 1 now
    rotate_ro=False,
    thresholds=None,
    average_exponent=12,
    sb_length=None,
    sb_range=None,
    sb_amplitude=None,
):

    # Load device and config params
    qubit_params_module = load_qubit_params(qubit_params_file_path)
    lo_settings = qubit_params_module.create_lo_settings(serial_num)
    readout_pulse = qubit_params_module.readout_pulse
    qubit_parameters = qubit_params_module.__dict__["qubit_parameters"]
    kernels = qubit_params_module.acquire_kernel

    ge_X180 = qubit_params_module.ge_X180
    ef_X180 = qubit_params_module.ef_X180
    sb_f0g1_readout = qubit_params_module.sb_pulses["readout"]["f0g1"]

    lo = lo_settings["q0"][serial_num]["SG2_LO"]
    lo_range = 0.5e9
    if freq_swp.start < lo - lo_range or freq_swp.stop > lo + lo_range:
        old_lo = lo
        new_lo = (freq_swp.start + freq_swp.stop) / 2
        step = 200e6
        new_lo = round(new_lo / step) * step        
        if new_lo < 1e9:
            new_lo = 0
        lo_settings["q0"][serial_num]["SG2_LO"] = new_lo
        lo = new_lo
        print(f"Warning: LO frequency changed to {new_lo/1e9} GHz")
        lo_change=True
    freq_swp.start -= lo
    freq_swp.stop -= lo


    if sb_length is None:
        sb_length = sb_f0g1_readout.length 
    if sb_amplitude is not None:
        sb_f0g1_readout.amplitude = 1
    if sb_range is None:
        sb_range = qubit_parameters["q0"]["sb_readout_dBm_range"]

    # Create Experiment
    exp = Experiment(
        uid=exp_id,
        signals=[
            ExperimentSignal("qb_drive"),
            ExperimentSignal("qb_ef_drive"),
            ExperimentSignal("sb_drive_readout"),
            ExperimentSignal("measure"),
            ExperimentSignal("acquire"),
        ],
    )
    with exp.acquire_loop_rt(
        uid="shots", count=pow(2, average_exponent), acquisition_type=acquisition_type
    ):
        with exp.sweep(
            uid="spect_sweep", parameter=freq_swp, reset_oscillator_phase=True
        ):
            with exp.section(uid="ge_excitation", play_after=None):
                exp.play(signal="qb_drive", pulse=ge_X180)

            with exp.section(
                uid="ef_excitation_1", play_after="ge_excitation", on_system_grid=True
            ):
                exp.play(signal="qb_ef_drive", pulse=ef_X180)

            with exp.section(uid="sb_transition_f0g1", play_after="ef_excitation_1"):
                exp.play(
                    signal="sb_drive_readout",
                    pulse=sb_f0g1_readout,
                    length=sb_length,
                    amplitude = sb_amplitude if sb_amplitude is not None else None
                )

            with exp.section(
                uid="ef_excitation_2",
                play_after="sb_transition_f0g1",
                on_system_grid=True,
            ):
                exp.play(signal="qb_ef_drive", pulse=ef_X180)

            with exp.section(uid="readout", play_after="ef_excitation_2"):
                exp.measure(
                    measure_signal="measure",
                    measure_pulse=readout_pulse,
                    acquire_signal="acquire",
                    integration_kernel=kernels,
                    handle="ac_0",
                    reset_delay=qubit_parameters["q0"]["reset_delay"],
                    acquire_delay=qubit_parameters["q0"]["acquire_delay"],
                )

    # setup calibration and signal map for the experiment
    sig_freq_map = create_default_map_and_calibration(
        exp,
        serial_num,
        qubit_parameters,
        lo_settings,
        rotate_ro=rotate_ro,
        thresholds=thresholds,
    )

    ch = "SG3"
    sig_freq_map[serial_num][ch] = {}
    sig_freq_map[serial_num][ch]["sb_drive_readout"] = {}
    sig_freq_map[serial_num][ch]["sb_drive_readout"]["frequency"] = freq_swp
    sig_freq_map[serial_num][ch]["sb_drive_readout"]["amplitude"] = sb_amplitude 
    sig_freq_map[serial_num][ch]["sb_drive_readout"]["range"] = sb_range
    sig_freq_map[serial_num][ch]["sb_drive_readout"]["length"] = sb_length

    print(sig_freq_map[serial_num]['SG3']['sb_drive_readout'])


    exp_calibration, map_q0 = default_signal_map_and_calibration(
        sig_freq_map,
        {
            "device_setup": device_setup,
            "lo_settings": lo_settings,
            "qubit_parameters": qubit_parameters,
        },
    )
    exp.set_calibration(exp_calibration)
    exp.set_signal_map(map_q0)

    return exp, lo
