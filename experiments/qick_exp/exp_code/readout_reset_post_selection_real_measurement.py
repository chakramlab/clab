...
class ReadoutResetPostSelectionRealMeasurementProgram(RAveragerProgram):
    ...
    def play_pige_pulse(self, phase = 0, shift = 0):
        ...
        self.sigma_ge = self.us2cycles(self.cfg.device.soc.qubit.pulses.pi_ge.sigma, gen_ch=self.qubit_ch)
    ...
    def body(self):
        ...
        if self.cfg.expt.prep == 'e':
            self.play_pige_pulse()
            self.sync_all()
        elif self.cfg.expt.prep == 'f':
            self.play_pige_pulse()
            self.sync_all()
        ...
        self.measure(pulse_ch=self.cfg.device.soc.resonator.ch,
                     ...
                     wait=True,
                     syncdelay=self.us2cycles(self.cfg.expt.reset_wait_time))  # sync all channels

        self.set_pulse_registers(
            ch=self.res_ch,
            style="const",
            freq=self.f_res,
            phase=self.deg2reg(self.cfg.device.soc.resonator.phase + self.cfg.expt.reset_phase, gen_ch=self.res_ch),
            gain=self.cfg.expt.reset_gain,
            length=self.us2cycles(self.cfg.expt.reset_length))
        
        self.pulse(ch=self.res_ch)  # Play reset pulse
        self.sync_all(self.us2cycles(self.cfg.expt.wait_time))

        if self.cfg.expt.prep == 'f':
            print('Playing pi_ef')
            self.play_pief_pulse()
            self.sync_all()
...
class ReadoutResetPostSelectionRealMeasurementExperiment(Experiment):
    """Ramsey Experiment
       Experimental Config
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        ramsey = ReadoutResetPostSelectionRealMeasurementProgram(soc, self.cfg)
        ...
        i_prob, q_prob = self.get_qubit_prob(avgi[0], avgq[0], iq_calib['i_g'], iq_calib['q_g'], iq_calib['i_e'], iq_calib['q_e'])

        data_dict = {'xpts': xpts, 'avgq':avgq[0], 'avgi':avgi[0], 'i_g': [iq_calib['i_g']], 'q_g': [iq_calib['q_g']], 'i_e': [iq_calib['i_e']], 'q_e': [iq_calib['q_e']], 'avgi_prob': i_prob, 'avgq_prob': q_prob}
