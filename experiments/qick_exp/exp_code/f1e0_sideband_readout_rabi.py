...
class f1e0ReadoutRabiProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        # Sideband pulse, two drives spaced by delta

        self.set_pulse_registers(
            ch=self.qubit_ch,
            style="flat_top",
            freq=self.freq2reg(cfg.expt.freq+cfg.expt.delta),
...
class f1e0ReadoutRabiExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None, prob_calib=True):
        ...
        for length in tqdm(lengths, disable=not progress):
            ...
            rspec = f1e0ReadoutRabiProgram(soc, self.cfg)
            self.prog = rspec
