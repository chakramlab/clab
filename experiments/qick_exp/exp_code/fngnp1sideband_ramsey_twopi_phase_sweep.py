...
class fngnp1SidebandRamseyTwoPiPhaseSweepProgram(AveragerProgram):
    def initialize(self):
        ...
        self.add_bump_func_freq_modulation(ch=self.sideband_ch, name='sb_flat_top_bump_freq_mod', ramp_length=self.us2cycles(self.cfg.expt.ramp_sigma), 
                                           flat_top_length=self.us2cycles(self.cfg.device.soc.sideband.pulses.fngnp1twopi_times[self.cfg.expt.mode][self.cfg.expt.n]), k=2, 
    ...
    def body(self):
        ...
        if self.cfg.expt.n==0:
            ...
            for i in np.arange(cfg.expt.n+1):
                ...
                if i == 0:
                    ...
                    self.play_pige_pulse(shift = i*chi_e)
                    self.sync_all()
        ...
        if self.cfg.expt.play_twopi:
            ...
            if self.cfg.device.soc.sideband.drive_frame_stark_shift_correction:
                self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type,ramp_sigma=sb_ramp_sigma, stark_shift_idle_correction=True)
            else:
                self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type,ramp_sigma=sb_ramp_sigma)
        ...
        for ii in np.arange(cfg.expt.n-1, -1, -1):
            ...
            self.play_pief_pulse(shift=ii * chi_ef)
            self.sync_all()

            # Play pi_ge pulse

            self.play_pige_pulse(shift=ii * chi_e)
            self.sync_all()
