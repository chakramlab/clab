...
class BeamsplitterSpectroscopyProgram(RAveragerProgram):
    def initialize(self):
    ...
    def play_sb(self, freq= 1, length=1, gain=1, ramp_type='sin_squared', ramp_sigma=1, phase=0, shift=0):
    ...
    def body(self):
        ...
        # setup and play pi_f0g1 on mode 1
        ...
        print('Playing beamsplitter pulse, freq = ' + str(sb_freq) + ', length = ' + str(sb_sigma) + ', gain = ' + str(sb_gain), ', ramp_sigma = ' + str(sb_ramp_sigma))
        self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, ramp_type=sb_ramp_type,ramp_sigma=sb_ramp_sigma)
        self.sync_all()

        # setup and play beamsplitter probe pulse
...
class BeamsplitterSpectroscopyExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        qspec_ef=BeamsplitterSpectroscopyProgram(soc, self.cfg)
