...
class SidebandOptimalControlQubitCavityTomographyPrepTestProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        self.play_pige_pulse(shift=chi_e)
        self.sync_all()

        self.play_piby2ef(shift=chi_ef)
        self.sync_all()
...
class SidebandOptimalControlQubitCavityTomographyPrepTestExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for i,j in tqdm(zip(cavdr_gains, cavdr_phases), total=len(cavdr_gains), disable = not progress):
            ...
            wigtom=SidebandOptimalControlQubitCavityTomographyPrepTestProgram(soc, self.cfg)
