...
class RamseyNOONStateStabilizationStabilizePhaseManualProgram(AveragerProgram):
    def initialize(self):
...
class RamseyNOONStateStabilizationStabilizePhaseManualExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for ii in tqdm(range(self.cfg.expt.expts), total=len(range(self.cfg.expt.expts))):
            ...
            avgi_temp = []
            avgq_temp = []
            for jj in self.cfg.expt.stabilize_phases:
                print('Stabilize Phase:', jj)
                self.cfg.expt.stabilize_phases_temp = jj
                soc = QickConfig(self.im[self.cfg.aliases.soc].get_cfg())
                wigtom=RamseyNOONStateStabilizationStabilizePhaseManualProgram(soc, self.cfg)
