...
class PNRQSSidebandOptimalControlPostSelectionProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        if self.cfg.expt.cond_f:
            print('Conditioned on |f>')
...
class PNRQSSidebandOptimalControlPostSelectionExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for i in tqdm(xpts, disable = not progress):
            ...
            qspec=PNRQSSidebandOptimalControlPostSelectionProgram(soc, self.cfg)
        ...
        for i in tqdm(xpts, disable = not progress):
            ...
            i_shots, q_shots = qspec.collect_shots()
            i_shots_col_cond_f.append(i_shots)
            q_shots_col_cond_f.append(q_shots)
            avgi_col_cond_f.append(avgi[0])
            avgq_col_cond_f.append(avgq[0])  
        ...
        avgi_col_cond_f = np.array(avgi_col_cond_f)
        avgq_col_cond_f = np.array(avgq_col_cond_f)
        ...
        data_dict = {
            ...
            'i_shots': i_shots_col, 
            'q_shots': q_shots_col,
            'avgi_cond_f': avgi_col_cond_f, 
            'avgq_cond_f': avgq_col_cond_f,
            'i_shots_cond_f': i_shots_col_cond_f, 
            'q_shots_cond_f': q_shots_col_cond_f
        }
