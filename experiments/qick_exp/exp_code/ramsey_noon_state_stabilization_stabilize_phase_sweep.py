...
class RamseyNOONStateStabilizationStabilizePhaseSweepProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        wait_time_frac = cfg.expt.length_placeholder/(self.cfg.expt.stabilize_cycles+1)

        for ii in range(self.cfg.expt.stabilize_cycles):
            stabilize_phase = self.cfg.expt.stabilize_phases_temp[ii]
            
            # Wait time
            
            self.sync_all(self.us2cycles(wait_time_frac))  # Wait for time tau/n
        
            # Parity protocol

            self.play_piby2ge()
            self.sync_all(self.us2cycles(cfg.expt.wait_time))  # Wait for time tau = chi/2
            ...
            self.play_pi2fh_pulse(phase=stabilize_phase)
            self.sync_all()
            ...
            if cfg.expt.ancilla_reset:
                print('Initiating ancilla reset')
        ...
        self.sync_all(self.us2cycles(wait_time_frac))

       # Decode NOON State

        for ii in range(self.cfg.expt.n-1, -1, -1):
            ...
            if ii == 0:
                ...
                self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type, ramp_sigma=sb_ramp_sigma, phase=self.cfg.expt.phase_placeholder)
                self.sync_all()
            ...
            if i == 0:
                ...
                # Step 2
                # theta_ef
                self.play_pief_pulse()  # pi_ef pulse to map populations onto transmon
                self.sync_all()
...
class RamseyNOONStateStabilizationStabilizePhaseSweepExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for ii in tqdm(range(self.cfg.expt.expts), total=len(range(self.cfg.expt.expts))):
            ...
            avgi_temp = []
            avgq_temp = []
            stabilize_phases_temp = []
            stabilize_phases = [self.cfg.expt.stabilize_phases for ii in range(self.cfg.expt.stabilize_cycles)]
            stabilize_phases = list(itertools.product(*stabilize_phases))
            print('Stabilize Phases:', stabilize_phases)
            for jj in stabilize_phases:
                print('Stabilize Phase:', jj)
                self.cfg.expt.stabilize_phases_temp = jj
                stabilize_phases_temp.append(jj)
                soc = QickConfig(self.im[self.cfg.aliases.soc].get_cfg())
                wigtom=RamseyNOONStateStabilizationStabilizePhaseSweepProgram(soc, self.cfg)
                avgi, avgq = wigtom.acquire(self.im[self.cfg.aliases.soc], threshold=None,load_pulses=True,progress=False)
            
                avgi_temp.append(avgi[0][0])
                avgq_temp.append(avgq[0][0])        
        
            avgi_col.append(avgi_temp)
            avgq_col.append(avgq_temp)
            stabilize_phases_list.append(stabilize_phases_temp)
        ...
        data_dict = {
            'xpts': lengths,
            'phases': phases,
            'stabilize_phases': stabilize_phases_list,
