...
class QubitPulseBangBangProgram(AveragerProgram):
    def initialize(self):
    ...
    def play_pige_pulse(self, phase = 0, shift = 0, gain_offset=0):
        ...
        if self.cfg.device.soc.qubit.pulses.pi_ge.pulse_type == 'const':
            ...
            self.set_pulse_registers(
                    ...
                    gain=self.cfg.device.soc.qubit.pulses.pi_ge.gain + gain_offset, 
    ...
    def body(self):
        ...
        # Repeatedly play pi_ge

        for ii in np.arange(self.cfg.expt.n_placeholder):

            self.play_pige_pulse(shift = self.cfg.expt.freq_offset_placeholder, gain_offset = self.cfg.expt.gain_offset_placeholder)
            self.sync_all()
...
class QubitPulseBangBangExperiment(Experiment):
        ...
        for freq_offset in tqdm(self.cfg.expt.freq_offsets, total=len(self.cfg.expt.freq_offsets)):
            for gain_offset in self.cfg.expt.gain_offsets:
                ...
                for ii in tqdm(n_pts, disable=not progress):
                    ...
                    lenrabi = QubitPulseBangBangProgram(soc, self.cfg)
