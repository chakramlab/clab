...
class fngnp1RabiReadoutPulsedResetProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        if self.cfg.expt.prep_state == 'e': 
            print('Preparation State: e')
            # pi_ge
                
            self.play_pige_pulse(phase=0) 
            self.sync_all()

        elif self.cfg.expt.prep_state == 'f':
            print('Preparation State: f')
            # pi_ge
            ...
            self.sync_all()
        elif self.cfg.expt.prep_state == 'epf':
            print('Preparation State: epf')
            # pi_ge
            ...
            self.play_pi2ef_pulse(phase=0)
            self.sync_all()
        else:
            print('Invalid Preparation State String (cfg.prep_state)')

        if self.cfg.expt.play_reset_cycle:
            ...
            self.sync_all(self.us2cycles(self.cfg.expt.length_placeholder))
...
class fngnp1RabiReadoutPulsedResetExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None):
        ...
        for length in tqdm(lengths, disable=not progress):
            ...
            lenrabi = fngnp1RabiReadoutPulsedResetProgram(soc, self.cfg)
            self.prog=lenrabi
