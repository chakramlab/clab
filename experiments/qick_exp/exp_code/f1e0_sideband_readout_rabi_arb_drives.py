...
class f1e0ReadoutRabiArbitraryDrivesProgram(AveragerProgram):
    def initialize(self):
...
class f1e0ReadoutRabiArbitraryDrivesExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None, prob_calib=True):
        ...
        for length in tqdm(lengths, disable=not progress):
            ...
            rspec = f1e0ReadoutRabiArbitraryDrivesProgram(soc, self.cfg)
            self.prog = rspec
