...
class f0g1SidebandRamseyPhaseSweepPiPulseTrainProgram(AveragerProgram):
    def initialize(self):
    ...
    def play_pige_pulse(self, gain=0, phase = 0, shift = 0):
        ...
        if self.cfg.device.soc.qubit.pulses.pi_ge.pulse_type == 'const':
            ...
            self.set_pulse_registers(
                    ...
                    gain=self.cfg.device.soc.qubit.pulses.pi_ge.gain+gain, 
    ...
    def play_pief_pulse(self, gain = 0, phase = 0, shift = 0):
        ...
        if self.cfg.device.soc.qubit.pulses.pi_ef.pulse_type == 'const':
            ...
            self.set_pulse_registers(
                    ...
                    phase=self.deg2reg(phase),
                    gain=self.cfg.device.soc.qubit.pulses.pi_ef.gain+gain, 
    ...
    def body(self):
        ...
        if self.pulse_type_ef == 'const':
            ...
            self.set_pulse_registers(
                    ...
                    phase=self.deg2reg(self.cfg.expt.phase_temp),
        ...
        if self.cfg.expt.n_train_bool:
            for ii in range(self.cfg.expt.n_train):
            ...
            for ii in range(self.cfg.expt.n_train):
                self.play_pief_pulse(gain=-self.cfg.device.soc.qubit.pulses.pi_ef.gain)
                self.sync_all()
                self.play_pige_pulse(gain=-self.cfg.device.soc.qubit.pulses.pi_ge.gain)
                self.sync_all()
                self.play_pief_pulse(gain=-self.cfg.device.soc.qubit.pulses.pi_ef.gain)
                self.sync_all()
...
class f0g1SidebandRamseyPhaseSweepPiPulseTrainExperiment(Experiment):
    """T1 Experiment
       Experimental Config
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for phase in tqdm(xpts, disable = not progress):
            ...
            t1 = f0g1SidebandRamseyPhaseSweepPiPulseTrainProgram(soc, self.cfg)
