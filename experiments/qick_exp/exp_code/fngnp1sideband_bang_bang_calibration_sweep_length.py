...
class fngnp1BangBangSweepLengthProgram(AveragerProgram):
    def initialize(self):
        ...
        self.add_bump_func(ch=self.sideband_ch, name="sb_flat_top_bump_offset", length=self.us2cycles(self.cfg.expt.sb_sigma) * 2, k=1 + self.cfg.expt.ramp_param_offset_placeholder)
        self.add_bump_func(ch=self.sideband_ch, name="sb_flat_top_bump", length=self.us2cycles(self.cfg.expt.sb_sigma) * 2, k=1)
    ...
    def play_sb(self, freq= 1, length=1, gain=1, pulse_type='flat_top', ramp_type='sin_squared', ramp_sigma=0.01, phase=0, shift=0, gain_shift=0, bump_offset=False):
        ...
        if pulse_type == 'const':
            ...
            if ramp_type == 'sin_squared':
                ...
                if bump_offset:
                    print('Sideband flat top bump with offset')
                    self.set_pulse_registers(
                        ch=self.sideband_ch,
                        style="flat_top",
                        freq=self.freq2reg(freq+shift),
                        ...
                        gain=gain,
                        length=self.us2cycles(length),
                        waveform="sb_flat_top_bump_offset")
                else:
    ...
    def body(self):
        ...
        for i in np.arange(cfg.expt.n):
            ...
            print('Freq: ', freq, 'Length: ', length, 'Gain: ', gain, 'Pulse type: ', pulse_type, 'Ramp type: ', ramp_type, 'Ramp sigma: ', ramp_sigma, bump_offset=False)
            self.play_sb(
                freq=freq,
                length=length,
                gain=gain,
        ...
        if self.cfg.expt.relative_phase_bool:
            ...
            for ii in np.arange(self.cfg.expt.n_placeholder):
                self.set_pulse_registers(
                    ch=self.sideband_ch,
                    style="flat_top",
                    freq=self.freq2reg(self.cfg.device.soc.sideband.fngnp1_freqs[self.cfg.expt.mode][self.cfg.expt.n]),
                    phase=self.deg2reg(phase),
                    gain=self.cfg.device.soc.sideband.pulses.fngnp1pi_gains[self.cfg.expt.mode][self.cfg.expt.n],
                    length=self.us2cycles(self.cfg.device.soc.sideband.pulses.fngnp1pi_times[self.cfg.expt.mode][self.cfg.expt.n] + self.cfg.expt.length_offset_placeholder),
                    waveform="sb_flat_top_bump_offset")
            ...
            print('Sideband pi pulse initiated:', 'Freq:', self.cfg.device.soc.sideband.fngnp1_freqs[self.cfg.expt.mode][self.cfg.expt.n], 
                ...
                'Gain:', self.cfg.device.soc.sideband.pulses.fngnp1pi_gains[self.cfg.expt.mode][self.cfg.expt.n],
                'Length offset:', self.cfg.expt.length_offset_placeholder, 'Ramp param offset:', self.cfg.expt.ramp_param_offset_placeholder)
        ...
        if self.cfg.expt.reset_f: 
            ...
            self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type, ramp_sigma=sb_ramp_sigma, bump_offset=False)
            self.sync_all()
        ...
        if cfg.expt.reset:
            ...
            for ii in range(cfg.device.soc.readout.reset_cycles):
                ...
                for ii in range(self.cfg.device.soc.readout.reset_cavity_n-1, -1, -1):
                    ...
                    print('Playing sideband pulse, freq = ' + str(sb_freq) + ', length = ' + str(sb_sigma) + ', gain = ' + str(sb_gain), ', ramp_sigma = ' + str(sb_ramp_sigma), sb_ramp_type)
                    self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type,ramp_sigma=sb_ramp_sigma, bump_offset=False)
                    self.sync_all()
...
class fngnp1BangBangSweepLengthExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None):
        ...
        avgi_col = []  # 2D list with I data of element [i, j] corresponding to [dataset with length_offset and param_param_offset, element in n_pts], 
                       # needs to be reshaped outside into a 3D array where the first two dimensions are [i, j]=[length_offset, param_param_offset]
        avgq_col = []
        i_prob_col = []
        q_prob_col = []
        ...
        for length_offset in tqdm(self.cfg.expt.length_offsets, total=len(self.cfg.expt.length_offsets)):
            for ramp_param_offset in self.cfg.expt.ramp_param_offsets:
                ...
                for ii in tqdm(n_pts, disable=not progress):
                    ...
                    self.cfg.expt.length_offset_placeholder = length_offset
                    self.cfg.expt.ramp_param_offset_placeholder = ramp_param_offset
                    print('number of sideband pulses:', ii)
                
                    soc = QickConfig(self.im[self.cfg.aliases.soc].get_cfg())

                    lenrabi = fngnp1BangBangSweepLengthProgram(soc, self.cfg)
        ...
        data_dict = {'n_pts': n_pts, 'length_offsets': self.cfg.expt.length_offsets, 'ramp_param_offsets': self.cfg.expt.ramp_param_offsets, 'avgq':avgq_col, 'avgi':avgi_col, 'i_g': [iq_calib['i_g']], 'q_g': [iq_calib['q_g']], 'i_e': [iq_calib['i_e']], 'q_e': [iq_calib['q_e']], 'avgi_prob': i_prob, 'avgq_prob': q_prob}
