...
def optimizing_iq_rotation(i, q):
    '''
    ...
    return theta, i_rot, q_rot


def iq_rotation(x_sweep, i, q, show=True):
    '''
    Rotate I and Q to maximize contrast in I
...
class PhaseRamseyAnalysis():
    ...
    def analyze(self):
        ...
        osc_freq = p[1]
        t2 = p[3]
        offset =  osc_freq - self.ramsey_freq
...
class PhaseRamseyAnalysisEF():
    def __init__(self, time, i, q, config=None):
        ...
        if not config:
            print('Error: no config file provided.')
        self.config = config
        self.qubit_freq = config['device']['soc']['qubit']['f_ef']
        step = self.config['expt']['step']
        phase_step = self.config['expt']['phase_step']
        self.ramsey_freq = phase_step/360/step
...
class PhaseRamseyAnalysisFH():
    def __init__(self, time, i, q, config=None):
        ...
        if not config:
            print('Error: no config file provided.')
        self.config = config
        self.qubit_freq = config['device']['soc']['qubit']['f_fh']
        step = self.config['expt']['step']
        phase_step = self.config['expt']['phase_step']
        self.ramsey_freq = phase_step/360/step
