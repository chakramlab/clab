...
class fngnp1BangBangSweepRampLengthProgram(AveragerProgram):
    def initialize(self):
        ...
        self.add_bump_func(ch=self.sideband_ch, name="sb_flat_top_bump_offset", length=self.us2cycles(self.cfg.expt.sb_sigma) * 2, k=2, flat_top_fraction=0.1 + self.cfg.expt.ramp_param_offset_placeholder)
        self.add_bump_func(ch=self.sideband_ch, name="sb_flat_top_bump", length=self.us2cycles(self.cfg.expt.sb_sigma) * 2, k=2, flat_top_fraction=0.1)
...
class fngnp1BangBangSweepRampLengthExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None):
        ...
        for length_offset in tqdm(self.cfg.expt.length_offsets, total=len(self.cfg.expt.length_offsets)):
            for ramp_param_offset in self.cfg.expt.ramp_param_offsets:
                ...
                print('length_offset:', length_offset, 'ramp_param_offset:', ramp_param_offset)
                avgi_col_temp = []
                avgq_col_temp = []
                for ii in tqdm(n_pts, disable=not progress):
                    ...
                    lenrabi = fngnp1BangBangSweepRampLengthProgram(soc, self.cfg)
