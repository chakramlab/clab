...
class RamseyCavityDisplacementCalibrationProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        if self.cfg.expt.pi2_sign_temp == 'positive':
            self.play_piby2ge(phase=0) 
            self.sync_all()
        
        if self.cfg.expt.pi2_sign_temp == 'negative':
            self.play_piby2ge(phase=180)
            self.sync_all()
...
class RamseyCavityDisplacementCalibrationExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for i,j in tqdm(zip(cavdr_gains, cavdr_phases), total=len(cavdr_gains), disable = not progress):
            ...
            wigtom=RamseyCavityDisplacementCalibrationProgram(soc, self.cfg)
            avgi, avgq = wigtom.acquire(self.im[self.cfg.aliases.soc], threshold=None,load_pulses=True,progress=progress)

            avgi_pi2_col.append(avgi[0][0])
            avgq_pi2_col.append(avgq[0][0])       
        ...
        for i,j in tqdm(zip(cavdr_gains, cavdr_phases), total=len(cavdr_gains), disable = not progress):
            ...
            avgi_neg_pi2_col.append(avgi[0][0])
            avgq_neg_pi2_col.append(avgq[0][0])  
        
        # Calibrate qubit probability

        avgi_pi2_col = np.array(avgi_pi2_col)
        avgq_pi2_col = np.array(avgq_pi2_col)
        avgi_neg_pi2_col = np.array(avgi_neg_pi2_col)
        avgq_neg_pi2_col = np.array(avgq_neg_pi2_col)

        iq_calib = self.qubit_prob_calib(path=self.path, config_file=self.config_file)
    
        i_prob, q_prob = self.get_qubit_prob(avgi_pi2_col, avgq_pi2_col, iq_calib['i_g'], iq_calib['q_g'], iq_calib['i_e'], iq_calib['q_e'])
        i_prob_neg, q_prob_neg = self.get_qubit_prob(avgi_neg_pi2_col, avgq_neg_pi2_col, iq_calib['i_g'], iq_calib['q_g'], iq_calib['i_e'], iq_calib['q_e'])

        data_dict = {'cavdr_gains': cavdr_gains, 'cavdr_phases': cavdr_phases, 'avgq':avgq_pi2_col, 'avgi':avgi_pi2_col, 'avgi_neg': avgi_neg_pi2_col, 'avgq_neg': avgq_neg_pi2_col, 
                     'i_g': [iq_calib['i_g']], 'q_g': [iq_calib['q_g']], 'i_e': [iq_calib['i_e']], 'q_e': [iq_calib['q_e']], 
                     'avgi_prob': i_prob, 'avgq_prob': q_prob, 'avgi_neg_prob': i_prob_neg, 'avgq_neg_prob': q_prob_neg}
