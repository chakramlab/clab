...
class T1FStabilizedf1e0Program(AveragerProgram):
    def initialize(self):
            ...
            self.add_gauss(ch=self.qubit_ch, name="qubit_pi", sigma=self.us2cycles(self.cfg.device.soc.qubit.pulses.pi_ge.sigma), length=self.us2cycles(self.cfg.device.soc.qubit.pulses.pi_ge.sigma) * 4)
    ...
    def body(self):
        ...
        # Sideband pulse, two drives 

        self.set_pulse_registers(
            ch=self.qubit_ch,
            style="flat_top",
            freq=self.freq2reg(cfg.device.soc.qubit.f_ef + cfg.expt.delta + cfg.expt.offset),
        ...
        self.set_pulse_registers(
            ch=self.sideband_ch,
            style="flat_top",
            freq=self.freq2reg(self.cfg.device.soc.qubit.f_ef + self.cfg.expt.delta + self.cfg.expt.offset),
            phase=self.deg2reg(0),
            gain=self.cfg.expt.gain1,
        ...
        self.set_pulse_registers(
            ch=self.qubit_ch,
            style="flat_top",
            freq=self.freq2reg(cfg.device.soc.resonator.freq - cfg.expt.delta),
            phase=self.deg2reg(0),
...
class T1FStabilizedf1e0Experiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None, prob_calib=True):
        ...
        for length in tqdm(lengths, disable=not progress):
            ...
            rspec = T1FStabilizedf1e0Program(soc, self.cfg)
            self.prog = rspec
