...
class NOONStateCoherenceQubitTomographySweepPhaseProgram(AveragerProgram):
    def initialize(self):
...
class NOONStateCoherenceQubitTomographySweepPhaseExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        phases = [self.cfg.expt.phase_start+ii*self.cfg.expt.phase_step for ii in range(self.cfg.expt.expts)]
        print('Phases:', phases)

        for ii in tqdm(phases, total=len(phases)):
            self.cfg.expt.phase_placeholder = ii
            soc = QickConfig(self.im[self.cfg.aliases.soc].get_cfg())
            wigtom=NOONStateCoherenceQubitTomographySweepPhaseProgram(soc, self.cfg)
