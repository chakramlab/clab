...
class WignerTomographyBinomialEncodingPostSelectionProgram(AveragerProgram):
    def initialize(self):
...
class WignerTomographyBinomialEncodingPostSelectionExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for i,j in tqdm(zip(cavdr_gains, cavdr_phases), total=len(cavdr_gains), disable = not progress):
            ...
            wigtom=WignerTomographyBinomialEncodingPostSelectionProgram(soc, self.cfg)
