...
class RamseyGFPiPulseTrainProgram(AveragerProgram):
    def initialize(self):
    ...
    def play_twopige(self, phase = 0, shift = 0):
        ...
        if self.cfg.device.soc.qubit.pulses.pi_ge.pulse_type == 'const':
            ...
            self.set_pulse_registers(
                    ...
                    freq=self.freq2reg(self.cfg.device.soc.qubit.f_ge + shift), 
                    phase=self.deg2reg(phase),
                    gain=self.cfg.device.soc.qubit.pulses.pi_ge.gain*2, 
                    length=self.sigma_ge)
    ...
    def play_twopief_pulse(self, phase = 0, shift = 0):
        ...
        if self.cfg.device.soc.qubit.pulses.pi_ef.pulse_type == 'const':
            ...
            self.set_pulse_registers(
                    ...
                    freq=self.freq2reg(self.cfg.device.soc.qubit.f_ef + shift), 
                    phase=self.deg2reg(phase),
                    gain=self.cfg.device.soc.qubit.pulses.pi_ef.gain*2, 
                    length=self.sigma_ef)
    ...
    def body(self):
        ...
        print(f'Playing {self.cfg.expt.n_pts_temp} pi_gf pulses')

        if self.cfg.expt.twopi:
            for ii in np.arange(self.cfg.expt.n_pts_temp):
                self.play_pief_pulse()
                self.sync_all()
                self.play_twopige()
                self.sync_all()
...
class RamseyGFPiPulseTrainExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None):
        ...
        x_pts = self.cfg.expt['n_pts']
        avgi_col = []
        avgq_col = []
        for i in tqdm(x_pts, disable = not progress):
            self.cfg.expt.n_pts_temp = i
            soc = QickConfig(self.im[self.cfg.aliases.soc].get_cfg())
            ramsey_echo = RamseyGFPiPulseTrainProgram(soc, self.cfg)
