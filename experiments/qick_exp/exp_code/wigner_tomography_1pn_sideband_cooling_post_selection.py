...
class WignerTomographyFock1pNSidebandCoolingPostSelectionProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        sb_freq = self.cfg.device.soc.sideband.fngnp1_freqs[self.cfg.expt.mode][1]
        sb_sigma = self.cfg.device.soc.sideband.pulses.fngnp1pi_times[self.cfg.expt.mode][1]
        sb_gain = self.cfg.device.soc.sideband.pulses.fngnp1pi_gains[self.cfg.expt.mode][1]
        sb_ramp_sigma = self.cfg.device.soc.sideband.pulses.fngnp1pi_ramp_sigmas[self.cfg.expt.mode][1]
        sb_ramp_type = self.cfg.device.soc.sideband.pulses.fngnp1pi_ramp_types[self.cfg.expt.mode]
        sb_pulse_type = self.cfg.device.soc.sideband.pulses.fngnp1pi_pulse_types[self.cfg.expt.mode]
        # print('Playing sideband pulse, freq = ' + str(sb_freq) + ', length = ' + str(sb_sigma) + ', gain = ' + str(sb_gain), ', ramp_sigma = ' + str(sb_ramp_sigma), ', ramp_type = ' + str(sb_ramp_type))
        ...
        sb_freq = self.cfg.device.soc.sideband.fngnp1_readout_freqs[0] + 2 * self.cfg.device.soc.storage.chi_f[self.cfg.expt.mode]
        sb_sigma = self.cfg.device.soc.sideband.pulses.fngnp1_readout_reset_lengths[0]
        sb_gain = self.cfg.device.soc.sideband.pulses.fngnp1_readout_gains[0]
        sb_pulse_type = self.cfg.device.soc.sideband.pulses.fngnp1_readout_pulse_types[0]
        sb_ramp_type = self.cfg.device.soc.sideband.pulses.fngnp1_readout_ramp_types[0]
...
class WignerTomographyFock1pNSidebandCoolingPostSelectionExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for i,j in tqdm(zip(cavdr_gains, cavdr_phases), total=len(cavdr_gains), disable = not progress):
            ...
            wigtom=WignerTomographyFock1pNSidebandCoolingPostSelectionProgram(soc, self.cfg)
        ...
        data_dict = {'cavdr_gains': cavdr_gains, 'cavdr_phases': cavdr_phases, 'i_g': i_g, 'q_g': q_g, 'i_e': i_e, 'q_e': q_e, 'i_f':i_f, 'q_f':q_f, 'avgi': avgi_col, 'avgq': avgq_col, 'i_shots': i_shots_col, 'q_shots': q_shots_col}
