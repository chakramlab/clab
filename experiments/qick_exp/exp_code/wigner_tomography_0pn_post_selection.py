...
class WignerTomographyFock0pNPostSelectionProgram(AveragerProgram):
    def initialize(self):
...
class WignerTomographyFock0pNPostSelectionExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for i,j in tqdm(zip(cavdr_gains, cavdr_phases), total=len(cavdr_gains), disable = not progress):
            ...
            wigtom=WignerTomographyFock0pNPostSelectionProgram(soc, self.cfg)
