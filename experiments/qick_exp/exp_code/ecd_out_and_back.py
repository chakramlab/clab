...
class ECDOutAndBackProgram(AveragerProgram):
    def initialize(self):
    ...
    def play_cavity_drive(self, gain=0, phase=0):
        ...
        if self.cfg.expt.cavity_drive_pulse_type == 'const':
            ...
            self.set_pulse_registers(
                    ch=self.cavdr_ch, 
                    style="const", 
                    freq=self.freq2reg(self.cfg.expt.cavity_drive_freq, gen_ch=self.cavdr_ch),
    ...
    def body(self):
        ...
        for kk in range(self.cfg.expt.num_displacements):
            self.play_cavity_drive(gain=self.cfg.expt.gain_temp, phase=0)
            self.sync_all(self.us2cycles(self.cfg.expt.wait_time))
            self.play_cavity_drive(gain=self.cfg.expt.gain_temp, phase=180+self.cfg.expt.phase_temp)

        # pi_ge
            
        self.play_pige_resolved_pulse() 
        self.sync_all()
...
class ECDOutAndBackExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None):
        ...
        for ii in tqdm(self.cfg.expt.cavity_drive_phases, disable=not progress):
            avgi_col_temp = []
            avgq_col_temp = []
            for jj in self.cfg.expt.cavity_drive_gains:
                self.cfg.expt.phase_temp = ii
                self.cfg.expt.gain_temp = jj
                
            
                soc = QickConfig(self.im[self.cfg.aliases.soc].get_cfg())

                lenrabi = ECDOutAndBackProgram(soc, self.cfg)
        ...
        data_dict = {'cavity_drive_gains': self.cfg.expt.cavity_drive_gains, 'cavity_drive_phases': self.cfg.expt.cavity_drive_phases, 'avgq':avgq_col, 'avgi':avgi_col, 'i_g': [iq_calib['i_g']], 'q_g': [iq_calib['q_g']], 'i_e': [iq_calib['i_e']], 'q_e': [iq_calib['q_e']], 'avgi_prob': i_prob, 'avgq_prob': q_prob}
