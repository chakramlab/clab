...
class QubitEFPulseBangBangProgram(AveragerProgram):
    def initialize(self):
    ...
    def play_pief_pulse(self, phase = 0, shift = 0, gain_offset = 0):
        ...
        if self.cfg.device.soc.qubit.pulses.pi_ef.pulse_type == 'const':
            ...
            self.set_pulse_registers(
                    ...
                    phase=self.deg2reg(phase),
                    gain=self.cfg.device.soc.qubit.pulses.pi_ef.gain + gain_offset, 
    ...
    def body(self):
        ...
        # Repeatedly play pi_ef

        for ii in np.arange(self.cfg.expt.n_placeholder):

            self.play_pief_pulse(shift = self.cfg.expt.freq_offset_placeholder, gain_offset = self.cfg.expt.gain_offset_placeholder)
            self.sync_all()
...
class QubitEFPulseBangBangExperiment(Experiment):
        ...
        for freq_offset in tqdm(self.cfg.expt.freq_offsets, total=len(self.cfg.expt.freq_offsets)):
            for gain_offset in self.cfg.expt.gain_offsets:
                ...
                for ii in tqdm(n_pts, disable=not progress):
                    ...
                    lenrabi = QubitEFPulseBangBangProgram(soc, self.cfg)
