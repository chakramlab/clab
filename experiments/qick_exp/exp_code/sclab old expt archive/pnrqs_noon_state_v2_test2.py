...
class PNRQSNOONStateProgram(AveragerProgram):
    def initialize(self):
        ...
        if self.cfg.expt.sb_pulse_type == 'flat_top':
            for ii in range(self.cfg.expt.n):
                self.add_gauss(ch=self.sideband_ch, name="sb_flat_top_"+str(self.cfg.expt.mode1)+"_"+str(ii), sigma=self.us2cycles(self.cfg.expt.sb_ramp_sigma), length=self.us2cycles(self.cfg.expt.sb_ramp_sigma) * 4)
                self.add_gauss(ch=self.sideband_ch, name="sb_flat_top_"+str(self.cfg.expt.mode2)+"_"+str(ii), sigma=self.us2cycles(self.cfg.expt.sb_ramp_sigma), length=self.us2cycles(self.cfg.expt.sb_ramp_sigma) * 4)
        self.add_gauss(ch=self.sideband_ch, name="sb_flat_top_pi2", sigma=self.us2cycles(cfg.expt.sb_ramp_sigma), length=self.us2cycles(cfg.expt.sb_ramp_sigma) * 4)
    ...
    def play_pi_sb(self, mode=0, n=0, phase=0, shift=0):
        if self.cfg.expt.sb_pulse_type == 'const':
            ...
            self.set_pulse_registers(
                    ch=self.sideband_ch, 
                    style="const", 
                    freq=self.freq2reg(self.cfg.device.soc.sideband.fngnp1_freqs[mode][n]+shift), 
                    phase=self.deg2reg(phase),
                    gain=self.cfg.device.soc.sideband.pulses.fngnp1pi_gains[mode][n], 
                    length=self.us2cycles(self.cfg.device.soc.sideband.pulses.fngnp1pi_times[mode][n]))
        ...
        if self.cfg.expt.sb_pulse_type == 'flat_top':
            ...
            self.set_pulse_registers(
                ...
                gain=self.cfg.device.soc.sideband.pulses.fngnp1pi_gains[mode][n],
                length=self.us2cycles(self.cfg.device.soc.sideband.pulses.fngnp1pi_times[mode][n]),
                waveform="sb_flat_top_"+str(mode)+"_"+str(n))
        
        self.pulse(ch=self.sideband_ch)
    
    def play_pi2_sb(self):
        if self.cfg.expt.sb_pulse_type == 'const':
            ...
            self.set_pulse_registers(
                    ch=self.sideband_ch, 
                    style="const", 
                    freq=self.freq2reg(self.cfg.device.soc.sideband.fngnp1_freqs[self.cfg.expt.mode1][0]), 
                    phase=self.deg2reg(0),
                    gain=self.cfg.device.soc.sideband.pulses.fngnp1pi_gains[self.cfg.expt.mode1][0], 
                    length=self.us2cycles(self.cfg.expt.sb_mode1_sigma))
    ...
    def play_pige_resolved_pulse(self, phase = 0, shift = 0):
        ...
        print('probe pulse type:', self.qubit_pulsetype)
    ...
    def body(self):
        ...
        self.play_pi_sb(mode=cfg.expt.mode2, n=0)
        self.sync_all()
        ...
        print('chi_e:', chie_mode1, chie_mode2)
        print('chi_f:', chif_mode1, chif_mode2)
        print('chi_ef:', chi_ef_mode1, chi_ef_mode2)

        for ii in range(1, self.cfg.expt.n):

            print('playing pi_ge in loop')
            self.play_pige_pulse(phase = 0, shift = ii*(chie_mode1 + chie_mode2)/2.0)
            self.sync_all()
            print('playing pi_ef in loop')
            self.play_pief_pulse(phase = 0, shift = ii*(chi_ef_mode1 + chi_ef_mode2)/2.0)
            self.sync_all()

            # pi_f(ii)g(ii+1) on Mode 1
            print('First sideband in loop')
            self.play_pi_sb(mode=cfg.expt.mode1, n=ii)
            self.sync_all()

            # pi_f(ii)g(ii+1) on Mode 2
            print('Second sideband in loop')
            self.play_pi_sb(mode=cfg.expt.mode2, n=ii)
            self.sync_all()
