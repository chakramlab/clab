...
class f1e0ReadoutSpectroscopyArbitraryDrivesProgram(RAveragerProgram):
    def initialize(self):
        ...
        self.f_start = self.freq2reg(cfg.device.soc.qubit.f_ef + cfg.expt.delta + cfg.expt.start)
        ...
        self.sb_rp=self.ch_page(self.sideband_ch)     # get register page for qubit_ch
        self.r_freq=self.sreg(self.sideband_ch, "freq")   # get frequency register for qubit_ch 
        self.r_freq2 = 4
        self.safe_regwi(self.sb_rp, self.r_freq2, self.f_start)  # set r_freq2 to start frequency
    ...
    def body(self):
        ...
        self.set_pulse_registers(
            ...
            phase=self.deg2reg(0),
            gain=self.cfg.expt.gain1,
            length=self.us2cycles(cfg.expt.length),
            waveform="sb_flat_top_bump")

        self.mathi(self.sb_rp, self.r_freq, self.r_freq2, "+", 0)    # +Delta freq
        self.pulse(ch=self.sideband_ch)
        ...
        self.set_pulse_registers(
            ch=self.qubit_ch,
            style="flat_top",
            freq=self.freq2reg(cfg.device.soc.resonator.freq - cfg.expt.delta),    # -Delta freq
            phase=self.deg2reg(0),
            gain=self.cfg.expt.gain2,
            length=self.us2cycles(cfg.expt.length),
        ...
        self.sync_all(self.us2cycles(self.cfg.expt.relax_time_before_meas))
        print('Relaxing before readout')
    ...
    def update(self):
        self.mathi(self.sb_rp, self.r_freq2, self.r_freq2, '+', self.f_step) # update frequency list index
        

class f1e0ReadoutSpectroscopyArbitraryDrivesExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        qspec_ef=f1e0ReadoutSpectroscopyArbitraryDrivesProgram(soc, self.cfg)
