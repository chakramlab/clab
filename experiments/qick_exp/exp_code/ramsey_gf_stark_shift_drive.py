...
class RamseyGFStarkShiftDriveProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        # print('test:', cfg.expt.tau_placeholder)
        self.play_sb(freq=self.cfg.expt.drive_freq, length=cfg.expt.tau_placeholder, gain=cfg.expt.drive_gain, pulse_type='const', ramp_sigma=self.cfg.device.soc.sideband.pulses.fngnp1_readout_ramp_sigmas[0])
        self.sync_all()
...
class RamseyGFStarkShiftDriveExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None):
        ...
        for i in tqdm(np.arange(len(x_pts)), disable = not progress):
            ...
            ramsey_echo = RamseyGFStarkShiftDriveProgram(soc, self.cfg)
