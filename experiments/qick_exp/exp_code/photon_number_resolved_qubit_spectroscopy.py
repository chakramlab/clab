...
class PhotonNumberResolvedQSpecProgram(RAveragerProgram):
    def initialize(self):
        ...
        self.qubitdr_freq_start = self.freq2reg(cfg.expt.start, gen_ch = self.q_ch)
        self.qubitdr_freq_step = self.freq2reg(cfg.expt.step)
        ...
        if self.cfg.expt.cavdr_pulse_type == 'gauss':
            ...
            self.add_gauss(ch=self.cavdr_ch, name="cavdr", sigma=self.us2cycles(self.cfg.expt.length), length=self.us2cycles(self.cfg.expt.length)* 4)
    ...
    def body(self):
        ...
        self.play_cavity_drive(gain=self.cfg.expt.gain, length=self.cfg.expt.length, phase=self.cfg.expt.phase)
        self.sync_all()
    ...
    def update(self):
        self.mathi(self.q_reg_page, self.q_freq_reg, self.q_freq_reg, '+', self.qubitdr_freq_step) # update frequency list index

class PhotonNumberResolvedQSpecExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        qspec=PhotonNumberResolvedQSpecProgram(soc, self.cfg)
