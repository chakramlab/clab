...
class RamseyPostSelectionProgram(RAveragerProgram):
    ...
    def collect_shots(self):
        ...
        shots_i0 = self.di_buf[0].reshape((cfg.expt.expts, cfg.expt.reps, cfg.expt.n_meas)) / self.us2cycles(cfg.device.soc.readout.length - self.cfg.device.soc.readout.adc_trig_offset, ro_ch=self.cfg.device.soc.readout.ch[0])
        shots_q0 = self.dq_buf[0].reshape((cfg.expt.expts, cfg.expt.reps, cfg.expt.n_meas)) / self.us2cycles(cfg.device.soc.readout.length - self.cfg.device.soc.readout.adc_trig_offset, ro_ch=self.cfg.device.soc.readout.ch[0])
        return shots_i0, shots_q0
    
class RamseyPostSelectionExperiment(Experiment):
    """Ramsey Experiment
       Experimental Config
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        ramsey = RamseyPostSelectionProgram(soc, self.cfg)
        ...
        # print('avgi:', avgi)
        # print('shape:', np.shape(avgi))
        # print('type:', type(avgi))
        i_shot, q_shot = ramsey.collect_shots()
        ...
        # i_prob, q_prob = self.get_qubit_prob(avgi, avgq, iq_calib['i_g'], iq_calib['q_g'], iq_calib['i_e'], iq_calib['q_e'])
        data_dict = {'xpts': xpts, 'avgi':avgi[0], 'avgq':avgq[0], 'i_shots': i_shot, 'q_shots': q_shot, 'i_g': i_g, 'q_g': q_g, 'i_e': i_e, 'q_e': q_e}

        # data_dict = {'xpts': xpts, 'avgq':avgq, 'avgi':avgi, 'i_shots': i_shot, 'q_shots': q_shot, 'i_g': [iq_calib['i_g']], 'q_g': [iq_calib['q_g']], 'i_e': [iq_calib['i_e']], 'q_e': [iq_calib['q_e']], 'avgi_prob': i_prob, 'avgq_prob': q_prob}
