...
class SidebandOptimalControlDetuningQubitCavityTomographyPostSelectionTestWaitTimeProgram(AveragerProgram):
    def initialize(self):
        ...
        if self.cfg.expt.pulse_info_from_json == True:
            ...
            self.qubit_pi2_waittime = self.us2cycles(self.cfg.expt.waittime)
        ...
        for ii in self.cfg.expt.sb_detunings:
            # print('Detuning (MHz):', ii)
            ...
            g_sb_n = 1 / 2 / tau_sb_n
            pulse_time = 1 / 2 / np.sqrt((ii / 2)**2 + g_sb_n**2) - self.cfg.expt.ramp_effective_time
            # print('Detuning (MHz):', ii)
            # print('Adjusted 2pi pulse time (us):', pulse_time)
            self.add_bump_func_freq_modulation(
                ch=self.sideband_ch, 
                name=f'sb_flat_top_bump_freq_mod_detuning{ii}', 
                ramp_length=self.us2cycles(self.cfg.expt.sb_ramp_sigma), 
                flat_top_length=self.us2cycles(pulse_time), 
                k=2, 
                freq = self.cfg.device.soc.sideband.fngnp1_stark_shifts[self.cfg.expt.mode][self.cfg.expt.n] + self.sb_n_detuning + ii)  # Chirp from the undriven f0g1 frequency
        
        for ii in self.cfg.expt.prep_sb_detunings:
    ...
    def play_sb(self, freq= 1, length=1, gain=1, pulse_type='flat_top', ramp_type='bump', ramp_sigma=0.01, phase=0, shift=0, stark_shift_idle_correction=False, detuning=0):
            ...
            if pulse_type == 'flat_top':
                ...
                if ramp_type == 'bump':
                    ...
                    self.set_pulse_registers(
                        ...
                        phase=self.deg2reg(phase),
                        gain=gain,
                        waveform=f"sb_flat_top_bump_freq_mod_detuning{detuning}")
    ...
    def body(self):
        ...
        if self.cfg.expt['sb_opt_cntrl_prep']:
            
            print('Preparing state with sideband optimal control')

            qubit_thetas = self.cfg.expt.prep_qubit_thetas
            qubit_phis = self.cfg.expt.prep_qubit_phis
            sb_phis = self.cfg.expt.prep_sb_phis
            sb_detunings = self.cfg.expt.prep_sb_detunings
        ...
        # Wait time after state preparation

        self.sync_all(self.us2cycles(self.cfg.expt.state_prep_wait_time))
...
class SidebandOptimalControlDetuningQubitCavityTomographyPostSelectionTestWaitTimeExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        if self.cfg.expt.g_only:

            print('Post-selection on |g> state only')

            self.sideband_calib(
            path=self.path, 
            data_path=data_path, 
            filename=f'f0g1_spectroscopy_stark_shift_calibration_mode{self.cfg.expt.mode+1}',
            max_iterations=10, 
            max_delta_offset=0.015, 
            n_update=self.cfg.expt.n, 
            max_n_update=self.cfg.expt.n,
            ...
            for i,j in tqdm(zip(cavdr_gains, cavdr_phases), total=len(cavdr_gains), disable = not progress):
                ...
                wigtom=SidebandOptimalControlDetuningQubitCavityTomographyPostSelectionTestWaitTimeProgram(soc, self.cfg)
            ...
            #     print('Gain = ', i, 'Phase = ', j)
            #     soc = QickConfig(self.im[self.cfg.aliases.soc].get_cfg())
            #     wigtom=SidebandOptimalControlQubitCavityTomographyPostSelectionTestWaitTimeProgram(soc, self.cfg)
            #     avgi, avgq = wigtom.acquire(self.im[self.cfg.aliases.soc], threshold=None,load_pulses=True,progress=progress)
            #     i_shots, q_shots = wigtom.collect_shots()
