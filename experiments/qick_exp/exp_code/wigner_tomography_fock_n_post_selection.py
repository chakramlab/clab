...
class WignerTomographyFockNPostSelectionProgram(AveragerProgram):
    def initialize(self):
    ...
    def play_cavity_drive(self, gain = 0, length = 1, phase = 0):
        ...
        if self.cavdr_pulse_type == 'const':
            print ("cavdr_pulse_type = const, in pulse defintiion")
            self.set_pulse_registers(
                ch=self.cavdr_ch,
                style="const",
                freq=self.freq2reg(self.cavdr_freq, gen_ch=self.cavdr_ch),
                phase=self.deg2reg(phase, gen_ch=self.cavdr_ch), # 0 degrees
                gain=gain,
                length=self.us2cycles(length))

            
        if self.cavdr_pulse_type == 'gauss':
            print ("cavdr_pulse_type = gauss, in pulse defintiion")
    ...
    def body(self):
        ...
        for i in np.arange(cfg.expt.n):
            ...
            self.play_pige(shift = self.chi_e*i)
            self.sync_all()
        ...
        #     length=self.us2cycles(self.cavdr_length))
...
class WignerTomographyFockNPostSelectionExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for i,j in tqdm(zip(cavdr_gains, cavdr_phases), total=len(cavdr_gains), disable = not progress):
            ...
            wigtom=WignerTomographyFockNPostSelectionProgram(soc, self.cfg)
