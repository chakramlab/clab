...
class ReadoutResetPostSelectionProgram(RAveragerProgram):
    ...
    def body(self):
        ...
        self.set_pulse_registers(
            ...
            style="const",
            freq=self.f_res,
            phase=self.deg2reg(self.cfg.device.soc.resonator.phase, gen_ch=self.res_ch),
            gain=self.cfg.device.soc.resonator.gain,
            length=self.readout_length)
        
        self.pulse(ch=self.res_ch)  # Play readout pulse
        self.sync_all(self.us2cycles(self.cfg.expt.reset_wait_time))

        self.set_pulse_registers(
            ch=self.res_ch,
            style="const",
            freq=self.f_res,
            phase=self.deg2reg(self.cfg.expt.reset_phase, gen_ch=self.res_ch),
...
class ReadoutResetPostSelectionExperiment(Experiment):
    """Ramsey Experiment
       Experimental Config
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        ramsey = ReadoutResetPostSelectionProgram(soc, self.cfg)
