...
class fngnp1RabiReadoutProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        self.play_sb(freq=cfg.expt.freq, length=self.sigma_fngnp1, gain=cfg.expt.gain, ramp_type=self.cfg.expt.flat_top_type, ramp_sigma=self.cfg.expt.sb_sigma, phase=0)
        self.sync_all()
...
class fngnp1RabiReadoutExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None):
        ...
        for length in tqdm(lengths, disable=not progress):
            ...
            lenrabi = fngnp1RabiReadoutProgram(soc, self.cfg)
            self.prog=lenrabi
