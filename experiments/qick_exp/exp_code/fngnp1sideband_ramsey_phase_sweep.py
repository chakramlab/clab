...
class fngnp1SidebandRamseyPhaseSweepProgram(AveragerProgram):
    def initialize(self):
        ...
        self.add_bump_func_freq_modulation(ch=self.sideband_ch, name='sb_flat_top_bump_freq_mod', ramp_length=self.us2cycles(self.cfg.expt.ramp_sigma), 
                                           flat_top_length=self.us2cycles(self.cfg.device.soc.sideband.pulses.fngnp1pi_times[self.cfg.expt.mode][0]), k=2, freq = self.cfg.device.soc.sideband.f0g1_stark_shifts[self.cfg.expt.mode])
        ...
        if stark_shift_idle_correction: 
            ...
            if pulse_type == 'flat_top':
                ...
                if ramp_type == 'bump':
                    ...
                    print('Freq. modulation (MHz):', self.cfg.device.soc.sideband.f0g1_stark_shifts[self.cfg.expt.mode])
                    self.set_pulse_registers(
                        ch=self.sideband_ch,
                        style="arb",
                        freq=self.freq2reg(freq + shift - self.cfg.device.soc.sideband.f0g1_stark_shifts[self.cfg.expt.mode]),
                        phase=self.deg2reg(phase),
                        gain=gain,
    ...
    def body(self):
        ...
        if self.pulse_type_ge2 == 'const':
            ...
            self.set_pulse_registers(
                    ch=self.qubit_ch, 
                    style="const", 
                    freq=self.freq2reg(cfg.device.soc.qubit.f_ge + chi_e * self.cfg.expt.n), 
                    phase=0,
        ...
        if self.pulse_type_ef == 'const':
            ...
            self.set_pulse_registers(
                    ch=self.qubit_ch, 
                    style="const", 
                    freq=self.freq2reg(cfg.device.soc.qubit.f_ef + chi_ef * self.cfg.expt.n), 
                    phase=0,
                    gain=self.cfg.device.soc.qubit.pulses.pi_ef.gain, 
                    length=self.sigma_ef)
...
class fngnp1SidebandRamseyPhaseSweepExperiment(Experiment):
    """T1 Experiment
       Experimental Config
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for phase in tqdm(xpts, disable = not progress):
            ...
            t1 = fngnp1SidebandRamseyPhaseSweepProgram(soc, self.cfg)
