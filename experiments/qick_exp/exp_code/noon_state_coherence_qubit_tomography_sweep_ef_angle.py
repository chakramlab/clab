...
class NOONStateCoherenceSweepEFAngleProgram(AveragerProgram):
    def initialize(self):
    ...
    def play_thetaef_pulse(self, gain=0, length = 1, phase = 0, shift = 0):

        self.add_gauss(ch=self.qubit_ch, name="qubit_ef_theta", sigma=self.us2cycles(length), length=self.us2cycles(length) * 4)
    ...
    def body(self):
        ...
        for i in range(self.cfg.expt.n):
            ...
            if i == 0:
                ...
                # Step 2
                # theta_ef
                self.play_thetaef_pulse(gain=self.cfg.expt.gain_placeholder, length=self.cfg.expt.ef_length)
                self.sync_all()

                print('gain', self.cfg.expt.gain_placeholder, 'length', self.cfg.expt.ef_length)
            else:
                # Step (1+6i)
       ...
       # Decode NOON State
        ...
        for i in range(self.cfg.expt.n-1, -1, -1):
            ...
            if i == self.cfg.expt.n-1:
                ...
                self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type, ramp_sigma=sb_ramp_sigma, phase=0)
                self.sync_all()
...
class NOONStateCoherenceSweepEFAngleExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        gains = [self.cfg.expt.start + self.cfg.expt.step * ii for ii in range(self.cfg.expt.expts)]
        for ii in tqdm(range(self.cfg.expt.expts), total=self.cfg.expt.expts):
            self.cfg.expt.gain_placeholder = gains[ii]
            self.cfg.expt.tomography_pulsetype = 'I'
            soc = QickConfig(self.im[self.cfg.aliases.soc].get_cfg())
            wigtom=NOONStateCoherenceSweepEFAngleProgram(soc, self.cfg)
