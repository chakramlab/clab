...
class fngnp1SidebandRamseyTwoPiPhaseSweepDetuningProgram(AveragerProgram):
    def initialize(self):
        ...
        self.add_bump_func(ch=self.sideband_ch, name="sb_flat_top_bump", length=self.us2cycles(self.cfg.expt.ramp_sigma) * 2, k=2+self.ramp_k_offset, flat_top_fraction=self.ramp_flat_top_fraction)

        self.sb_n_detunings = np.array(self.cfg.device.soc.sideband.fngnp1_freqs[self.cfg.expt.mode]) - self.cfg.device.soc.sideband.fngnp1_freqs[self.cfg.expt.mode][0]
        
        for ii in range(0, self.cfg.expt.n+1):
            ...
            self.add_bump_func_freq_modulation(
                ...
                ramp_length=self.us2cycles(self.cfg.expt.ramp_sigma), 
                flat_top_length=self.us2cycles(self.cfg.device.soc.sideband.pulses.fngnp1pi_times[self.cfg.expt.mode][ii]), 
                k=2, 
                freq = self.cfg.device.soc.sideband.fngnp1_stark_shifts[self.cfg.expt.mode][ii] + self.sb_n_detunings[ii])
    ...
    def play_sb(self, n=0, pi=False, freq= 1, length=1, gain=1, pulse_type='flat_top', ramp_type='sin_squared', ramp_sigma=1, phase=0, shift=0, stark_shift_idle_correction=False):
                ...
                if ramp_type == 'bump':
                    ...
                    if pi==False:
                        ...
                        self.set_pulse_registers(
                            ch=self.sideband_ch,
                            style="arb",
                            freq=self.freq2reg(freq + shift - self.cfg.device.soc.sideband.fngnp1_stark_shifts[self.cfg.expt.mode][0] - self.sb_n_detunings[n]),
                            phase=self.deg2reg(phase),
                            gain=gain,
                        ...
                        self.set_pulse_registers(
                            ...
                            phase=self.deg2reg(phase),
                            gain=gain,
                            waveform=f"sb_flat_top_bump_freq_mod_pi_f{n}g{n+1}")
    ...
    def body(self):
        ...
        if self.cfg.expt.n==0:
            ...
            self.play_pief_pulse(phase=self.cfg.expt.phase_offset + self.cfg.expt.phase_temp)  # Phase offset to calibrate Ramsey start phase without a 2pi_f,n-g,n+1 pulse
            self.sync_all()
            ...
            for i in np.arange(cfg.expt.n+1):
                ...
                if i == 0:
                    ...
                    self.play_piby2ef(phase=self.cfg.expt.phase_offset + self.cfg.expt.phase_temp)  # Phase offset to calibrate Ramsey start phase without a 2pi_f,n-g,n+1 pulse
                    self.sync_all()
                ...
                if i != cfg.expt.n:
                    ...
                    self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type, ramp_sigma=sb_ramp_sigma, stark_shift_idle_correction=True, n=i, pi=True)
                    self.sync_all()

        if self.cfg.expt.play_twopi:
            # setup and play fn,gnp1 sideband twopi pulse
            
            for ii in range(self.cfg.expt.n_layers):
                sb_freq = self.cfg.device.soc.sideband.fngnp1_freqs[self.cfg.expt.mode][self.cfg.expt.n]
                sb_sigma = self.cfg.device.soc.sideband.pulses.fngnp1twopi_times[self.cfg.expt.mode][self.cfg.expt.n]
                ...
                if self.cfg.device.soc.sideband.drive_frame_stark_shift_correction:
                    self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type,ramp_sigma=sb_ramp_sigma, stark_shift_idle_correction=True, n=self.cfg.expt.n)
                else:
                    self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type,ramp_sigma=sb_ramp_sigma)
            ...
            for ii in range(self.cfg.expt.n_layers):
                ...
                sb_ramp_sigma = self.cfg.device.soc.sideband.pulses.fngnp1pi_ramp_sigmas[self.cfg.expt.mode][self.cfg.expt.n]
                sb_ramp_type = self.cfg.device.soc.sideband.pulses.fngnp1pi_ramp_types[self.cfg.expt.mode]
                # print('Playing empty sideband pulse, freq = ' + str(sb_freq) + ', length = ' + str(sb_sigma) + ', gain = ' + str(0), ', ramp_sigma = ' + str(sb_ramp_sigma))
        ...
        for ii in np.arange(cfg.expt.n-1, -1, -1):
            ...
            self.play_sb(freq=sb_freq, length=sb_sigma, gain=sb_gain, pulse_type=sb_pulse_type, ramp_type=sb_ramp_type, ramp_sigma=sb_ramp_sigma, stark_shift_idle_correction=True, n=ii, pi=True)
            self.sync_all()
...
class fngnp1SidebandRamseyTwoPiPhaseSweepDetuningExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for ii in tqdm(np.arange(len(xpts)), disable = not progress):
