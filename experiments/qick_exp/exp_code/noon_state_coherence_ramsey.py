...
class NOONStateCoherenceRamseyProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        self.sync_all(self.us2cycles(self.cfg.expt.length_placeholder))  # Wait time for the NOON state to decohere
...
class NOONStateCoherenceRamseyExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        phases = [ii*self.cfg.expt.phase_step for ii in range(self.cfg.expt.expts)]
        ...
        for ii in tqdm(range(self.cfg.expt.expts), total=len(range(self.cfg.expt.expts))):
            ...
            wigtom=NOONStateCoherenceRamseyProgram(soc, self.cfg)
