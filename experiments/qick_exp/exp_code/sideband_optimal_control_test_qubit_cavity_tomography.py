...
class SidebandOptimalControlTestQubitCavityTomographyProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        print('Length (us):', self.cfg.device.soc.sideband.pulses.fngnp1pi_times[self.cfg.expt.mode][0])
        self.play_sb(
                ...
                phase = 0,    
                stark_shift_idle_correction = False)
        self.sync_all()
...
class SidebandOptimalControlTestQubitCavityTomographyExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        for i,j in tqdm(zip(cavdr_gains, cavdr_phases), total=len(cavdr_gains), disable = not progress):
            ...
            wigtom=SidebandOptimalControlTestQubitCavityTomographyProgram(soc, self.cfg)
