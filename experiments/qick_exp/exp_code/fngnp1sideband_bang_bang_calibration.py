...
class fngnp1bangbangProgram(AveragerProgram):
    def initialize(self):
        ...
        try: self.freq_offset = self.cfg.expt.freq_offset
        except: self.freq_offset = 0
    ...
    def body(self):
        ...
        for i in np.arange(cfg.expt.expts):
            self.play_pi_sb(n=self.cfg.expt.n, shift = self.freq_offset)
            self.sync_all()
...
class fngnp1bangbangExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None):
        lengths = np.arange(self.cfg.expt["expts"])
