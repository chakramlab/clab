...
class ECDDisplacementAreaCalibrationPNRQSProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        ...
        if self.cfg.expt.prep == 'e':
            print('Preparing qubit in |e>')
            self.play_pige_pulse()
            self.sync_all()
        else:
            print('Preparing qubit in |g>')
...
class ECDDisplacementAreaCalibrationPNRQSExperiment(Experiment):
    ...
    def acquire(self, progress=False, data_path=None, filename=None):
        ...
        for ii in tqdm(self.cfg.expt.cavity_drive_gains, disable=not progress):
            ...
            for i in tqdm(fpts, disable = not progress):
                ...
                qspec=ECDDisplacementAreaCalibrationPNRQSProgram(soc, self.cfg)
        ...
        data_dict = {'cavity_drive_gains': self.cfg.expt.cavity_drive_gains, 'xpts':fpts, 'avgi':avgi_col, 'avgq':avgq_col, 'avgi_prob': i_prob, 'avgq_prob': q_prob}
