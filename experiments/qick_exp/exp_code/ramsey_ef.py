...
class RamseyEFProgram(RAveragerProgram):
    ...
    def initialize(self):
        ...
        self.add_gauss(ch=self.qubit_ch, name="qubit_pi", sigma=self.us2cycles(cfg.device.soc.qubit.pulses.pi_ge.sigma), length=self.us2cycles(cfg.device.soc.qubit.pulses.pi_ge.sigma) * 4)
        self.add_gauss(ch=self.qubit_ch, name="qubit_ef", sigma=self.us2cycles(self.cfg.device.soc.qubit.pulses.pi2_ef.sigma), length=self.us2cycles(self.cfg.device.soc.qubit.pulses.pi2_ef.sigma) * 4)
    ...
    def body(self):
        ...
        if self.cfg.device.soc.qubit.pulses.pi_ge.pulse_type == 'const':
            ...
            self.set_pulse_registers(
                ...
                length=self.us2cycles(cfg.device.soc.qubit.pulses.pi_ge.sigma))
        ...
        # Setup and play pi_ef/2 qubit pulse
        ...
        if self.cfg.device.soc.qubit.pulses.pi2_ef.pulse_type == 'const':
            ...
            self.set_pulse_registers(
                ...
                length=self.us2cycles(self.cfg.device.soc.qubit.pulses.pi2_ef.sigma))
...
class RamseyEFExperiment(Experiment):
    ...
    def acquire(self, progress=False, debug=False, data_path=None, filename=None):
        ...
        ramsey = RamseyEFProgram(soc, self.cfg)
